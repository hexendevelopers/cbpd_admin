...
const jobMarketInsightsMap: Record<string, { description: string, salaryRange: string, growthRate: string, topEmployers: string[] }> = {
  "business": {
  ...
  try {
    ...
    for (const cat of rawData) {
      ...
      const category = await CourseCategory.create({
        ...
        icon: cat.icon,
        image: cat.image,
        description: `Explore our specialized ${cat.title} designed for professional excellence.`,
        isActive: true
      });

      const insight = jobMarketInsightsMap[cat.slug] || {
        salaryRange: "£30,000 - £80,000+",
        growthRate: "Steady Growth",
        topEmployers: ["Leading UK Companies", "Global Enterprises", "SMEs"],
        description: `The demand for specialized expertise in this field is reaching an all-time high as the sector rapidly transforms.`
      };

      // Create courses (subcategories) for this category
      const coursesToCreate = cat.subs.map(sub => {
        const subSlug = sub.toLowerCase().replace(/ & | /g, "-").replace(/[^a-z0-9-]/g, "");
        return {
          title: sub,
          slug: subSlug,
          categoryId: category._id,
          description: `Master the principles of ${sub} with our globally accredited industry curriculum.`,
          overview: `Our rigorous ${sub} curriculum provides a definitive path to commanding ${cat.title.replace(/ Programmes?/i, '').toLowerCase()} principles in the modern industry landscape. You will gain profound insights designed and validated by senior experts.`,
          curriculum: [
      ...
      coursesAdded += coursesToCreate.length;
    }

    return NextResponse.json({
      success: true,
      message: `Database seeded successfully. Added ${rawData.length} Categories and ${coursesAdded} Courses.`
    });
