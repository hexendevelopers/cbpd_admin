...
export async function GET(request: NextRequest) {
  try {
    ...
    const exportData = students.map((student, index) => ({
      ...
      'Joining Date': student.joiningDate ? new Date(student.joiningDate).toLocaleDateString() : 'N/A',
      
      // Address Information
      'State': student.state || 'N/A',
      'District': student.district || 'N/A',
      'County': student.county || 'N/A',
      'Full Address': `${student.state || ''}, ${student.district || ''}, ${student.county || ''}`.replace(/^,\s*|,\s*$/g, '').replace(/,\s*,/g, ',') || 'N/A',
      ...
      'Contact Person': student.institutionId ? `${student.institutionId.firstName || ''} ${student.institutionId.lastName || ''}`.trim() : 'N/A',
      ...
      'Secondary Contact': student.institutionId && (student.institutionId.SfirstName || student.institutionId.SlastName) ? 
        `${student.institutionId.SfirstName || ''} ${student.institutionId.SlastName || ''}`.trim() : 'N/A',
      ...
      // File Information
      'Passport Photo': student.passportPhoto ? 'Uploaded' : 'Not Uploaded',
      'Marksheets Count': student.marksheets ? student.marksheets.length : 0,
      'Marksheets Status': student.marksheets && student.marksheets.length > 0 ? 'Uploaded' : 'Not Uploaded',
      
      // Status and Dates
      'Student Status': student.isActive ? 'Active' : 'Inactive',
      'Institution Status': student.institutionId?.isTerminated ? 'Terminated' : 
        (student.institutionId?.isApproved ? 'Approved' : 'Pending'),
      ...
      'Institution Created Date': student.institutionId?.createdAt ? new Date(student.institutionId.createdAt).toLocaleDateString() : 'N/A',
    }));
